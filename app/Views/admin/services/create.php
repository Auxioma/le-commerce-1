...
$pageSubtitle = 'Ce service apparaîtra sur la page publique « Nos Services » une fois publié.';
$pageActions = [];
...
<div class="max-w-2xl">
  ...
  <div class="card card-md sm:p-8">
    <form method="POST" action="<?= BASE_PATH ?>/admin/services" class="space-y-5">
      <?= Csrf::field() ?>

      <div>
        <label class="block text-sm font-semibold text-ink mb-1.5">Nom du service</label>
        <input type="text" name="name" required value="<?= htmlspecialchars($old['name'] ?? '') ?>" placeholder="Ex : Retrait de colis"
      ...
      <div>
        ...
        <textarea name="description" rows="2" placeholder="Ex : Dépôt et retrait de vos colis en point relais, sans rendez-vous."
      ...
      <div>
        ...
        <select name="icon" required class="form-select <?= isset($errors['icon']) ? 'border-brand-400' : '' ?>">
          ...
          <?php foreach ($iconChoices as $path => $label): ?>
            <option value="<?= htmlspecialchars($path) ?>" <?= ($old['icon'] ?? '') === $path ? 'selected' : '' ?>><?= htmlspecialchars($label) ?></option>
          <?php endforeach; ?>
        </select>
        <?php if (isset($errors['icon'])): ?><p class="text-brand-500 text-xs mt-1"><?= htmlspecialchars($errors['icon']) ?></p><?php endif; ?>
        <p class="text-xs text-gray-400 mt-1">Vous pourrez ajouter une photo dédiée depuis <a href="<?= BASE_PATH ?>/admin/images" class="text-brand-500 hover:underline">Photos du site</a> une fois le service enregistré (nécessite d'ajouter son emplacement dans le registre d'images).</p>
      </div>
      ...
      <label class="flex items-center gap-2.5 text-sm text-gray-600 bg-gray-50 rounded-2xl px-4 py-3">
        ...
        Publier immédiatement sur /nos-services (sinon le service est enregistré inactif)
      </label>
